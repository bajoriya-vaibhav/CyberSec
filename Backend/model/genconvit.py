...
class GenConViT(nn.Module):
    ...
    def __init__(self, config, ed, vae, net, fp16, weight_dir="weight"):
        super(GenConViT, self).__init__()
        self.net = net
        self.fp16 = fp16

        ed_path = os.path.join(weight_dir, f"{ed}.pth")
        vae_path = os.path.join(weight_dir, f"{vae}.pth")
        ...
        if self.net=='ed':
            try:
                ...
                raise Exception(f"Error: {ed_path} file not found.")
        elif self.net=='vae':
            try:
                ...
                raise Exception(f"Error: {vae_path} file not found.")
        else:
            try:
                self.model_ed = GenConViTED(config)
                self.model_vae = GenConViTVAE(config)
